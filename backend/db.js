...
db.serialize(() => {
    ...
    db.run(
        `INSERT INTO superusers (username, password, accountid) VALUES (?, ?, ?)`,
        [process.env.CONTRACT_OWNER_USERNAME, process.env.CONTRACT_OWNER_PASSWORD, process.env.CONTRACT_OWNER_ACCOUNT_ID],
        function (err) {
            if (err) {
                if (err.message.includes('UNIQUE constraint failed')) {
                    console.log('Superuser already exists.');
                } else {
                    console.error('Error inserting superuser:', err.message);
                }
            } else {
                console.log('Superuser created with ID:', this.lastID);
            }
        }
    );
});
